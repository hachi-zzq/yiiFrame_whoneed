...
<div class="pageHeader">
    <form rel="pagerForm" onsubmit="return navTabSearch(this);" action="/admin/channel_daily/time_search" method="post">
        <div class="searchBar" style="padding-top: 10px;">
            ...
            <a style="display: none" class="btnLook" id="app_look_up" href="/admin/channel_daily/appLookBack" lookupGroup="app_district">查找带回</a>
        </div>
        <div class="searchBar" style="padding-top: 10px;">
            <label>时间检索：</label>
            开始时间：
...
<div class="pageContent">
    ...
    <table class="table" width="900" layoutH="138">
        ...
        <tr>
            ...
            <?php if($type == 'inner'){?><th width="150" ><span class="tb_span" title="除去注册当天第二次登陆的用户数 / 注册数">二次登录率</span></th><?php } ?>
            ...
            foreach($all_father as $f){
                ...
                <tr target="sid_user" rel="1">
                    ...
                    <td><?php echo $f['channel_name']?>                                                                         <!--渠道名称-->
                        <a target="navTab" href="/admin/channel_daily/channelDailyChild?fid=<?php echo $f['channel_id']?>&type=<?php echo $type?>&start_time=<?php echo $time['start_time']?>&end_time=<?php echo $time['end_time']?>&pay_start_time=<?php echo $pay_time?$pay_time['pay_start_time']:date('Y-m-d')?>&pay_end_time=<?php echo $pay_time?$pay_time['pay_end_time']:date('Y-m-d')?>" style="color: green">
...
//                    $cost = $regCost*$f['reg_sum'];
                    ...
                    $APUR = $f['pay_users']==0?0:round($f['amount']/$f['pay_users'],4);
                    ...
                    <td><?php echo $f['pay_users']?$f['pay_users']:0?></td>                                                             <!--充值用户数-->
                    <td><?php echo $f['amount']?$f['amount']:0?></td>                                                                   <!--充值金额-->
                    <td><?php echo $APUR?></td>                                                                                         <!--APUR-->
                    <?php if($type=='inner') echo '<td>',$f['login2_nums'],'</td>'?>                                                    <!--二登数-->
                    <?php if($type=='inner') echo '<td>',$loginTwicePer*100,'%</td>'?>                                                  <!--第二次登录数-->
                ...
                $cost_nums +=$cost;
                $login2_nums +=$f['login2_nums'];
                $pay_users_nums +=$f['pay_users'];
        ...
        <tr>
            ...
            if($pay_users_nums == 0){
                ...
                $sumAPUR  = round($pay_num/$pay_users_nums,2);
            }
            ...
            <?php  if($type == 'inner') echo '<td>',$login2_nums,'</td>'?>
            <?php  if($type == 'inner') echo '<td>',$sumTwicePer,'</td>'?>
