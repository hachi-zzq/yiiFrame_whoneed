...
<div class="pageContent">
    <div class="panelBar">
        <ul class="toolBar">
            ...
            <li><a class="edit" href="/admin/data_channel/subChannelEdit?id={sid_user}" target="navTab" warn="请选择一个用户"><span>修改</span></a></li>
    ...
    <table class="table" width="800" layoutH="138">
        ...
        if(isset($all_child) && !empty($all_child)){
            foreach($all_child as $v){
                ...
                $channelName = Pdcc_channel::model()->find("id='{$channel_id}'")->channel_name;
                ?>
                <?php
                ...
                <tr target="sid_user" rel="<?php echo $v['id']?>">
                    ...
                    <td>
                        <a title="确定删除？" target="ajaxTodo" href="/admin/data_channel/subChannelMultiDel?ids=<?php echo $v['id']?>&fid=<?php echo $_GET['fid']?>">删除 </a>/
                        <a title="编辑" target="navTab" href="/admin/data_channel/subChannelEdit?id=<?php echo $v['id']?>&fid=<?php echo $_GET['fid']?>">编辑</a>
                        　<a <?php echo $style?> href="/admin/channel/channelDistribute?channel_name=<?php echo $channelName?>&channel_id=<?php echo $v['channel_id']?>&sub_id=<?php echo $v['sub_id']?>" target="navTab">渠道游戏包分发<?php echo $packageTitle?></a>
...
<script>
    function batchDel(){
        var strDUrl = '/admin/data_channel/subChannelMultiDel/ids/';
        var strIds = '';
