 ...
 */
class Channel_dailyController extends MyAdminController{
    // 初始化
    ...
    public function idStr(){
        ...
        $user_channel = Pdc_user_channel::model();
        ...
        if($all_user_channel){
            ...
            //不在表中       不进行权限控制
            $all_channel = Pdc_channel::model()->findAll();
    ...
    public function actionChannelDailyFather(){
        ...
        $sql = "select
                        ...
                        sum(login_2_nums)  as login2_nums ,
        ...
        foreach($all as &$v){
            ...
            $pays = Yii::app()->db_data_statistics->createCommand("SELECT sum(pay_amount) as sum_account FROM pds_pay_atom_statistics WHERE reg_time BETWEEN '$now_time_beg' AND '$now_time_end' AND pay_time BETWEEN '$now_time_beg' AND '$now_time_end' AND channel_id = '$channel_id'")->queryRow();
            ...
            //子渠道数目
            $count = $channel_model->count('fid=:f_id',array('f_id'=>$channel_id));
            $v['chlid_count'] = $count;
        }
        $data['all_father'] = $all;
        $data['pager'] = $pages;
...
//        $data['pay_time'] = array('pay_start_time'=>$now_time_beg,'pay_end_time'=>$now_time_end);
    ...
    public function actionChannelDailyChild(){
        ...
        //分页
        $cdb = new CDbCriteria();
        $cdb->condition = "channel_id='{$fid}' and record_date between '$start_time' and '$end_time'  and  channel_id in ($id_str) and sub_id != 0";
        $cdb->group = 'sub_id';
        $count = Pds_channel_daily::model()->count($cdb);
        $pages = new CPagination($count);
        $pages->pageSize = self::PAGE_SIZE;
        ...
        foreach($all_child as &$c){
            $id = $c['sub_id'];
            $sql = "SELECT id,sum(pay_amount) pay_amount FROM pds_pay_atom_statistics WHERE reg_time BETWEEN '$full_start_time' AND '$full_end_time' AND pay_time BETWEEN '$pay_start_time' AND '$pay_end_time' AND sub_id = '$id'AND channel_id = '$fid' ";
            $pays = Yii::app()->db_data_statistics->createCommand($sql)->queryRow();
            $child_pay_account = $pays['pay_amount'];
            $one = Pdc_channel::model()->find('id=:i_d and fid !=0',array('i_d'=>$id));
    ...
    public function actionTime_search(){
        ...
        //app_id
        $appId = Yii::app()->request->getParam('app_district_app_id');
        //过滤channel_from
        if($channel_from != '0'){
        ...
        //过滤app_name
        if(!empty($appId)){
            $arrId = array_intersect(explode(',',$id_str),$this->getChannelByApp($appId));
            $id_str = implode(',',$arrId);
            //ajaxPost分页条件传递
            $arrCondition['app_district_app_id'] = $appId;
        }
...
//        print_r($objChannel);
        //get pay data
        $pay_sql = "select sum(pay_amount) as pay_sum_amount,channel_id from pds_pay_atom_statistics where reg_time between '{$full_time_start}' and '{$full_time_end}' and pay_time between '{$pay_start_time}' and '{$pay_end_time}' GROUP BY channel_id ";
...
//        print_r($objPay);
        ...
        if($objChannel){
            foreach($objChannel as $channel){
                ...
                $child_count = $channel_model->count("fid = {$channel['channel_id']}");
...
//        print_r($arrChannel);
    ...
    public function filterChannelFrom($arrIds,$channelFrom){
        ...
        if(is_array($arrIds)){
            foreach($arrIds as $id){
                if(Pdc_channel::model()->find("id='{$id}'")->channel_from == $channelFrom)
    ...
    public function filterChannelType($arrIds,$channelType){
        ...
        if(is_array($arrIds)){
            foreach($arrIds as $id){
                if(Pdc_channel::model()->find("id='{$id}'")->channel_type == $channelType)
                    array_push($arrChannel,$id);
            }
        }
        return $arrChannel;
    ...
    //getChannelByApp
    public function getChannelByApp($appId){
        $arrChannel = array();
        $objChannel = Pdc_type::model()->findAll("appid='{$appId}'");
        foreach($objChannel as $channel){
            array_push($arrChannel,$channel['channel_id']);
        }
        return $arrChannel;
    }
