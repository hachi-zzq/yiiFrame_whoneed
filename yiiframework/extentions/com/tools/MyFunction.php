...
STR;
    ...
    static function getChannelByServerid($serverid){
        static $server_array =array();
        if( !isset($server_array[$serverid]) ){
            $channelInfo = false;
            ...
            $server_array[$serverid] = $channelInfo;
        }
        return $server_array[$serverid];
    }
    ...
    /*兼容函数
     *通过旧或新格式的serverid获取,新旧serverid
     *不验证serverid是否正确
     */
    static function getAllServerTypeByServerid($serverid){
        static $servers=array();
        if( !isset($servers[$serverid]) ){
            $server_array = array($serverid);
            //如果是旧格式,查出新格式
            if( is_numeric($serverid) && ( $channelInfo = self::getChannelInfo($serverid) ) ){
                //查父渠道信息
                $f_channel = Pdcc_channel::model()->getDbConnection()->createCommand("select a.id,channel_name,b.app_ab,b.appid,c.company_ab from pdcc_channel as a join pdcc_product as b on a.product_id=b.id join pdcc_company as c on b.company_id=c.id where a.id='".$channelInfo['channel_id']."'")->queryRow();
                if($f_channel){
                    $server_array[] = $f_channel['company_ab'].'_'.$f_channel['app_ab'].'_'.$channelInfo['channel_id'].'_'.$channelInfo['sub_id'];
                    if( $channelInfo['sub_id']==0 ){
                        $server_array[] = $f_channel['company_ab'].'_'.$f_channel['app_ab'].'_'.$channelInfo['channel_id'];
                    }
                }
            }else if( preg_match('/^([^_]+_[^_]+_([\d]+))(?:_([\d]+))?$/',$serverid,$result) ){
                if( !isset($result[3]) ){
                    $server_array[] = $serverid.'_0';
                }else if( $result[3]==0 ){
                    $server_array[] = $result[1];
                }
                //如果是新格式,则查是否存在旧格式
                $channel_id = $result[2];
                $sub_id     = isset($result[3]) ? $result[3] : 0;
                if( $sub_id==0 ){
                    $id  = $channel_id;
                    $fid = 0;
                }else{
                ...
                $channelObj = Pdc_channel::model()->find("id = {$id} and fid={$fid}");
                if($channelObj){
                    $server_array[] = $id;
                }
            }
            $servers[$serverid] = $server_array;
        }
